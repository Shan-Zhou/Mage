...
layout(binding = BINDING_VERTEX_LOCATIONS, set = 0, scalar) buffer VertexLocation
{
  uint vertexLocations[];
};
layout(binding = BINDING_INDEX_LOCATIONS, set = 0, scalar) buffer IndexLocation
{
  uint indexLocations[];
};

//layout(binding = BINDING_NORMALS, set = 0, scalar) buffer Normals
//{
//  vec3 normals[];
//};
...
void main()
{
  ...
  const vec3 v0 = vertices[vertexLocations[gl_InstanceCustomIndexEXT]/3 + indices[indexLocations[gl_InstanceCustomIndexEXT] + 3 * gl_PrimitiveID + 0]];
  const vec3 v1 = vertices[vertexLocations[gl_InstanceCustomIndexEXT]/3 + indices[indexLocations[gl_InstanceCustomIndexEXT] + 3 * gl_PrimitiveID + 1]];
  const vec3 v2 = vertices[vertexLocations[gl_InstanceCustomIndexEXT]/3 + indices[indexLocations[gl_InstanceCustomIndexEXT] + 3 * gl_PrimitiveID + 2]];
